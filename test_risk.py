...
class TestRiskManager:
    ...
    def test_position_sizing_fixed(self):
        """Test fixed position sizing (respects max position size clamp)"""
        config = {
            'risk': {
                'stop_loss_percent': 2,
                'take_profit_percent': 4,
        ...
        # 100 shares @ $100 = $10k, but max position = 5% of $100k = $5k
        # So clamped to 50 shares
        assert quantity == 50
        
    def test_position_sizing_fixed_no_clamp(self):
        """Fixed sizing with large enough max_position_size_percent"""
        config = {
            'risk': {
                'stop_loss_percent': 2,
                'take_profit_percent': 4,
        ...
        manager = RiskManager(config)
        quantity, details = manager.calculate_position_size(100.0, 100000.0)
        # 100 shares @ $100 = $10k, max = 20% of $100k = $20k — no clamp
        assert quantity == 100
    ...
    def test_position_sizing_zero_price(self):
        """Position sizing returns 0 for zero/negative entry price"""
        config = {
            'risk': {'stop_loss_percent': 2, 'take_profit_percent': 4, 'max_concurrent_positions': 3},
            'account': {'max_position_size_percent': 5, 'max_daily_loss_percent': 2},
            'position_sizing': {'method': 'risk_based', 'risk_per_trade_percent': 1},
        }
        manager = RiskManager(config)
        quantity, details = manager.calculate_position_size(0.0, 100000.0)
        assert quantity == 0

    def test_position_sizing_zero_balance(self):
        """Position sizing returns 0 for zero balance"""
        config = {
            'risk': {'stop_loss_percent': 2, 'take_profit_percent': 4, 'max_concurrent_positions': 3},
            'account': {'max_position_size_percent': 5, 'max_daily_loss_percent': 2},
            'position_sizing': {'method': 'risk_based', 'risk_per_trade_percent': 1},
        ...
        manager = RiskManager(config)
        quantity, details = manager.calculate_position_size(100.0, 0.0)
        assert quantity == 0

    def test_position_sizing_zero_stop_loss(self):
        """Position sizing returns 0 for zero stop_loss_percent"""
        config = {
            'risk': {'stop_loss_percent': 0, 'take_profit_percent': 4, 'max_concurrent_positions': 3},
            'account': {'max_position_size_percent': 5, 'max_daily_loss_percent': 2},
            'position_sizing': {'method': 'risk_based', 'risk_per_trade_percent': 1},
        }
        manager = RiskManager(config)
